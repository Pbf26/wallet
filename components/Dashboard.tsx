...
export default function Dashboard({ profile, transactions, goals, onSignOut }: Props) {
  ...
  // Cash = computed from all transactions (dynamic, always accurate)
  const totalCash = transactions.reduce((s, t) => t.type === 'income' ? s + t.amount : s - t.amount, 0)

  // Debts = total outstanding from profile (updated at onboarding, user manages manually)
  ...
  const recent = transactions.filter((t) => t.category !== 'Balance inicial').slice(0, 12)

  // Per-bank balance computed from transactions
  const bankBalances: Record<string, number> = {}
  for (const acc of (profile.bank_accounts || [])) {
    bankBalances[acc.bank] = 0
  }
  ...
  const hasBanks = (profile.bank_accounts || []).length > 0
        ...
        <div className="card" style={{ marginBottom: 12 }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 0 }}>
            <div style={{ textAlign: 'center', padding: '10px 6px' }}>
              ...
              <div style={{ fontSize: 15, fontWeight: 700, color: totalCash >= 0 ? '#16a34a' : '#dc2626' }}>{fmt(totalCash)}</div>
            </div>
            <div style={{ textAlign: 'center', padding: '10px 6px', borderLeft: '1px solid #e2e2de', borderRight: '1px solid #e2e2de' }}>
              ...
              <div style={{ fontSize: 15, fontWeight: 700, color: totalOwed > 0 ? '#dc2626' : '#888' }}>{fmt(totalOwed)}</div>
            </div>
            <div style={{ textAlign: 'center', padding: '10px 6px' }}>
              <div style={{ fontSize: 10, color: '#888', marginBottom: 4, fontWeight: 500 }}>NETO REAL</div>
              <div style={{ fontSize: 15, fontWeight: 700, color: netBalance >= 0 ? '#1a1a1a' : '#dc2626' }}>{fmt(netBalance)}</div>
            </div>
          </div>
          {totalOwed > 0 && (
            <div style={{ borderTop: '1px solid #f0f0ed', marginTop: 4, paddingTop: 8, fontSize: 11, color: '#aaa', textAlign: 'center' }}>
              {totalCardUsed > 0 && <span>Tarjetas: {fmt(totalCardUsed)}</span>}
              {totalCardUsed > 0 && totalDebt > 0 && <span> · </span>}
              {totalDebt > 0 && <span>Créditos: {fmt(totalDebt)}</span>}
            </div>
          )}
        </div>
            ...
            {(profile.bank_accounts || []).map((acc, i) => {
              ...
              return (
                <div key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingBottom: 8, marginBottom: 8, borderBottom: i < (profile.bank_accounts.length - 1) ? '1px solid #f0f0ed' : 'none' }}>
                  <div>
                    <div style={{ fontSize: 13, fontWeight: 500 }}>{acc.bank}</div>
                    <div style={{ fontSize: 11, color: '#aaa', marginTop: 1 }}>{acc.account_type}</div>
                  </div>
        ...
        {recent.length === 0 ? (
          ...
          <div className="card" style={{ padding: 0, overflow: 'hidden' }}>
            {recent.map((t, i) => {
              ...
              return (
                <div key={t.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '11px 14px', borderBottom: i < recent.length - 1 ? '1px solid #f0f0ed' : 'none' }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ fontSize: 11, color: '#aaa', marginTop: 1 }}>
                      {t.category}
                      {t.bank && <span style={{ color: '#888' }}> · {t.bank}</span>}
                      {t.payment_method && <span style={{ color: '#888' }}> {t.payment_method}</span>}
